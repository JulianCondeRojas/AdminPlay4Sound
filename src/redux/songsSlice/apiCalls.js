...
export const createSong = async (song, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.post("/api/songs", song);
...
export const updateSong = async (id, song, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.put(`/api/songs/${id}`, song);
...
export const deleteSong = async (id, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.delete(`/api/songs/${id}`);
